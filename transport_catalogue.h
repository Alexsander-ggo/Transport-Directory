...
namespace router::tc {
    ...
    public:
        using StopsDistance = std::unordered_map<std::pair<std::shared_ptr<Stop>,
                                                 std::shared_ptr<Stop>>, size_t, HashPairStops>;
        
        void                                                        AddBus(const Bus& bus);

        void                                                        AddStop(const Stop& stop);

        void                                                        SetDistanceBetweenStops(std::shared_ptr<Stop> from,
                                                                                            std::shared_ptr<Stop> to,
                                                                                            size_t distance);

        size_t                                                      GetDistanceBetweenStops(std::shared_ptr<Stop> from,
    ...
    private:
        ...
        std::unordered_map<std::string_view,
            std::shared_ptr<Bus>>                                   index_buses_;
        std::unordered_map<std::string_view,
            std::shared_ptr<Stop>>                                  index_stops_;
        std::unordered_map<std::shared_ptr<Bus>,
            BusStat>                                                info_of_buses_;
        std::unordered_map<std::shared_ptr<Stop>,
            Buses>                                                  buses_for_stops_;
